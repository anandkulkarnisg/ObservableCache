...
{
	std::shared_ptr<Callback> testPtr(new StreamEventListner<std::string, std::string>());
	testPtr->onTick(make_pair<std::string, std::string>("Anand", "Apparao Kulkarni"));   	

	std::shared_ptr<Callback> testPtr1(new StreamEventListner<std::string, std::string>());
	...
	std::shared_ptr<Callback> testPtr2(new FileEventListner<std::string, std::string>("output.txt"));
